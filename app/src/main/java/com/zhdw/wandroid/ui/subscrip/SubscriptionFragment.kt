...
import com.zhdw.wandroid.databinding.FragmentSubscriptionBinding
 ...
 * desc:
 */
class SubscriptionFragment : BaseFragmentBinding() {

    private lateinit var viewBinding: FragmentSubscriptionBinding
    ...
    override fun initViewBinding(): ViewBinding {
        viewBinding = FragmentSubscriptionBinding.inflate(layoutInflater)
        return viewBinding
    }

    override fun initData() {
        viewModel.mWxSubScription.observe(this, Observer {
            titleArray.clear()
            fragments.clear()
            for (sub in it) {
                titleArray.add(sub.name)
                fragments.add(
                    WxSubListFragment.instance(
                        sub.id
                    )
                )
            }
            ...
            viewBinding.subVP.adapter = vpAdapter
            ...
            commonNavigator.adapter = object : CommonNavigatorAdapter() {
                override fun getTitleView(context: Context?, index: Int): IPagerTitleView {
                    ...
                    title.setOnClickListener { viewBinding.subVP.currentItem = index }
                    return title
                }
            ...
            viewBinding.subVP.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
